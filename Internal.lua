...
local library = {windows = 0, elements = {}}
local ScreenGui = Instance.new("ScreenGui")

function library:Window(name)
    local window = {toggled = false, flags = {}, elements = {}}
    ...
    function window:Slider(name, min, max, default, callback)
        ...
        SliderBg.ImageColor3 = Color3.fromRGB(39, 39, 39)
        ...
        SliderFill.ImageColor3 = Color3.fromRGB(100, 150, 255)
        SliderFill.ScaleType = Enum.ScaleType.Slice
        SliderFill.SliceCenter = Rect.new(100, 100, 100, 100)
        SliderFill.SliceScale = 0.02

        SliderButton.Name = "SliderButton"
        SliderButton.Parent = SliderBg
        SliderButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        SliderButton.BackgroundTransparency = 0.000
        SliderButton.Position = UDim2.new((current - min) / (max - min), -8, -0.5, 0)
        ...
        ValueLabel.Position = UDim2.new(0, 10, 0, 33)
        ValueLabel.Size = UDim2.new(0, 160, 0, 14)
    ...
    function window:Dropdown(name, objects, callback)
        ...
        local function closeDropdown()
            toggled = false
            game:GetService("TweenService"):Create(Toggle, TweenInfo.new(0.35), {Rotation = 0}):Play()
            Main:TweenSize(UDim2.new(0, 180, 0, 0), "Out", "Sine", 0.35, true)
            wait(.35)
...
local lib = require(script.Parent.GradientLib) -- adjust path as needed

-- Create a window
local myWindow = lib:Window("My UI")

-- Add elements to window
myWindow:Toggle("Enable Feature", function(value)
    print("Toggle enabled: " .. tostring(value))
end)

myWindow:Button("Click Me", function()
    print("Button clicked!")
end)

myWindow:Slider("Speed", 0, 100, 50, function(value)
    print("Speed set to: " .. value)
end)

myWindow:Dropdown("Choose One", {"Option A", "Option B", "Option C"}, function(selected)
    print("Selected: " .. selected)
end)

myWindow:Box("Enter Text", "placeholder", function(text)
    print("You typed: " .. text)
end)

-- Access window elements if needed
print("Window flags:", myWindow.flags)
print("Window elements:", myWindow.elements)
]]