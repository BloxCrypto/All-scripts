...
mobileButton.DisplayOrder = 2147483647
mobileButton.Parent = (RunService:IsStudio() 
	and LocalPlayer:FindFirstChild("PlayerGui")
	or (gethui and gethui())
	or (cloneref and cloneref(MacLib.GetService("CoreGui")) or MacLib.GetService("CoreGui")))
...
toggleButton.Image = Settings.MobileButton and Settings.MobileButton.Image or "rbxassetid://10734950309" -- Default settings icon
toggleButton.ImageColor3 = Color3.fromRGB(255, 255, 255)
toggleButton.ScaleType = Enum.ScaleType.Fit
toggleButton.Parent = mobileButton
...
local function updateDrag(input)
	...
	Tween(toggleButton, TweenInfo.new(0.15), {Position = newPosition}):Play()
end
...
toggleButton.MouseButton1Click:Connect(function()
	if dragging then return end -- Don't toggle if dragging
	
	local currentState = WindowFunctions:GetState()
	WindowFunctions:SetState(not currentState)
	
	-- Visual feedback
	local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local scaleTween = Tween(toggleButton, tweenInfo, {Size = UDim2.fromOffset(55, 55)})
	scaleTween:Play()
	
	task.wait(0.1)
	
	local scaleBackTween = Tween(toggleButton, tweenInfo, {Size = UDim2.fromOffset(60, 60)})
	scaleBackTween:Play()
...
touchIndicator.Size = UDim2.fromScale(1.2, 1.2)
touchIndicator.Position = UDim2.fromScale(0.5, 0.5)
touchIndicator.AnchorPoint = Vector2.new(0.5, 0.5)
touchIndicator.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
touchIndicator.BackgroundTransparency = 1
touchIndicator.BorderSizePixel = 0
touchIndicator.ZIndex = 0
touchIndicator.Parent = toggleButton

local indicatorCorner = Instance.new("UICorner")
indicatorCorner.CornerRadius = UDim.new(1, 0)
indicatorCorner.Parent = touchIndicator

toggleButton.MouseButton1Down:Connect(function()
	touchIndicator.BackgroundTransparency = 0.7
	local tween = Tween(touchIndicator, TweenInfo.new(0.3), {BackgroundTransparency = 1})
	tween:Play()
end)

-- Store reference for cleanup
WindowFunctions.MobileButton = {
	Button = toggleButton,
	Container = mobileButton,
	SetVisibility = function(state)
		mobileButton.Enabled = state
	end,
...
local function isMobile()
	...
	-- Mobile if touch is enabled and no keyboard/mouse
...
local function getOptimalSize()
	...
	if isMobileDevice then
		-- Mobile: Use 90% of screen width, max height 80%
		local width = math.min(viewportSize.X * 0.9, 500)
		local height = math.min(viewportSize.Y * 0.8, 600)
		return UDim2.fromOffset(width, height)
	else
		-- PC: Use default size or custom size from Settings
		return Settings.Size or UDim2.fromOffset(868, 650)
	end
end
...
local function updateWindowSize()
	...
	Tween(base, tweenInfo, {Size = optimalSize}):Play()
end
...
local function updateUIScale()
	...
	if isMobileDevice then
		...
		local scale = math.clamp(minDimension / 600, 0.7, 1)
...
local function adjustSidebarForMobile()
	local isMobileDevice = isMobile()
	
	if isMobileDevice then
		-- On mobile, make sidebar narrower
		local newWidth = math.min(base.AbsoluteSize.X * 0.35, 200)
		...
		if base.AbsoluteSize.X < 400 then
			userInfo.Visible = false
		else
			userInfo.Visible = true
		end
	else
		-- PC: Use default sidebar width
		sidebar.Size = UDim2.new(0, defaultSidebarWidth, 1, 0)
		content.Size = UDim2.new(0, base.AbsoluteSize.X - defaultSidebarWidth, 1, 0)
		userInfo.Visible = true
	end
end

local function adjustMobileButtonVisibility()
	local isMobileDevice = isMobile()
	
	if WindowFunctions.MobileButton then
		-- Show mobile button only on mobile devices
		WindowFunctions.MobileButton.SetVisibility(isMobileDevice)
	end
	
	-- Hide drag icon on mobile (they use the button instead)
	if moveIcon then
		moveIcon.Visible = not isMobileDevice
	end
end
...
local function onViewportChanged()
	...
	adjustMobileButtonVisibility()
end

-- Initial setup
onViewportChanged()

-- Listen for viewport changes (rotation, window resize)
workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(onViewportChanged)

-- Listen for device input changes
UserInputService:GetPropertyChangedSignal("TouchEnabled"):Connect(onViewportChanged)
UserInputService:GetPropertyChangedSignal("KeyboardEnabled"):Connect(onViewportChanged)

-- Add orientation change detection for mobile
if isMobile() then
	game:GetService("Players").LocalPlayer:GetPropertyChangedSignal("DeviceOrientation"):Connect(function()
		task.wait(0.1) -- Small delay to let viewport update
		onViewportChanged()
	end)
end

-- Add responsive resize functions to WindowFunctions
function WindowFunctions:EnableResponsiveResize(enabled)
	if enabled then
		onViewportChanged()
	else
...
function WindowFunctions:ForceResize()
	onViewportChanged()
end

function WindowFunctions:IsMobileDevice()
	return isMobile()
end

function WindowFunctions:SetMobileSize(width, height)
	if isMobile() then
		base.Size = UDim2.fromOffset(width, height)
	end
end

function WindowFunctions:SetPCSize(width, height)
	...
	function WindowFunctions:Unload()
		...
		if WindowFunctions.MobileButton then
		WindowFunctions.MobileButton.Container:Destroy()
	end
		macLib:Destroy()
		unloaded = true
	end
