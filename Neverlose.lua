...
local function SetMobileOptimized(gui)
    if isMobile then
        gui.Size = UDim2.new(0, math.min(658, gui.Parent.AbsoluteSize.X - 20), 0, math.min(516, gui.Parent.AbsoluteSize.Y - 20))
    end
end
...
local function Dragify(frame, parent)
    ...
    input.InputChanged:Connect(function(input)
        if dragging then
            if input == dragInput or input == touchInput then
...
local function buttoneffect(options)
    pcall(function()
        ...
        if isMobile then
            options.entered.MouseButton1Down:Connect(function()
                TweenService:Create(options.frame, TweenInfo.new(0.06, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                    TextColor3 = Color3.fromRGB(234, 239, 245)
                }):Play()
            end)
            options.entered.MouseButton1Up:Connect(function()
                TweenService:Create(options.frame, TweenInfo.new(0.06, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                    TextColor3 = Color3.fromRGB(157, 171, 182)
                }):Play()
            end)
...
local function clickEffect(options)
    ...
    options.button.MouseButton1Click:Connect(trigger)
    
    -- Touch support
    options.button.TouchTap:Connect(trigger)
end
...
function Library:Window(options)
    ...
    -- Mobile optimization
    if isMobile then
        Body.Size = UDim2.new(0, math.min(658, 400), 0, math.min(516, 500))
    end
    ...
    function tabsections:TabSection(options)
        ...
        function tabs:Tab(options)
            ...
            newPage.ScrollBarThickness = isMobile and 8 or 4
                ...
                function elements:Slider(options)
                    ...
                    RS.RenderStepped:Connect(function()
                        if Held then
                            ...
                            local MousePos = isMobile and Mouse.X or UIS:GetMouseLocation().X
